...
This file runs the standalone browser version of the LOS calculator (no backend server).
Data flow: fetch `data.csv` -> parse rows with Papa Parse -> calculate stats in-memory -> render results/chart in the page.
All filtering and math happen client-side using JavaScript arrays.
*/
...
// Load CSV data
// Reads local CSV data and starts parsing so dropdowns/results can be generated.
async function loadCSVData() {
    try {
        ...
        Issue: CSV data is trusted without integrity/authentication checks.
        Why it matters: modified CSV content can silently change analytics results.
        Where: `fetch('data.csv')` and parse flow in `loadCSVData()`.
        Later fix idea: verify data checksum/signature and validate schema before use.
        */
...
// Process CSV data
// Cleans rows, skips metadata/footer entries, and converts year columns into numeric records.
...
// Populate dropdowns
// Builds unique category/location option lists from parsed in-memory records.
...
// Validates required input, runs LOS calculation, and shows rendered output.
...
// Applies selected filters and computes average/min/max plus yearly trend values.
...
// Display results
// Writes computed statistics into DOM placeholders and refreshes the chart.
...
// Create chart
// Uses Chart.js to render yearly averages as a line chart.
...
// UI Helper functions
// Shared helpers for loading/error/result visibility state.
